...
// Query to fetch allowed properties and their images
$query = "
    SELECT p.*, i.image 
    FROM property p
...
while ($row = $result->fetch_assoc()) {
    ...
    // Group property details and images
...
<head>
    ...
    $(document).ready(function() {
        $('#searchForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'GET',
                url: 'test2.php', // Your PHP script to handle the search
                data: $(this).serialize(), // Serialize form data
                success: function(response) {
                    $('#propertyResults').html(response); // Update the property results
                },
                error: function() {
...
<body>
   <div class="navbar">
        ...
        <a href="property_dislay.php">Home</a>
         <a href="update_property.php">My property</a>  
...
<div class="container">
    ...
    <div id="propertyResults" class="row">
        <?php
        // Check if any property is found
        if (count($properties) > 0) {
            ...
            foreach ($properties as $property) {
                ...
                echo "<div class='col-md-4'>"; // 3 properties per row
                ...
                echo "<input type='hidden' name='user_id' value='" . $uid . "' />"; // Assuming you store user ID in session
                echo "<button type='submit' class='request-rent-button'>Request Rent</button>";
                echo "</form>";
                echo "</div>";

                echo "</div>"; // End of property div
